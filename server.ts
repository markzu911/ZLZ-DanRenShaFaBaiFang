...
摆放逻辑：必须像真实室内设计师在现场布置。第一步先判断房间窗户、阳台、电视墙、通道、已有沙发/茶几/柜体的位置，并确定一个位于窗户边/落地窗边/窗帘旁采光区的固定沙发落点；第二步判断已有沙发的大体朝向，用靠背线、座面开口、扶手方向、抱枕方向和茶几/电视墙关系推断现有沙发正面向量；第三步把上传单人沙发的正面向量锁定为同一方向，只允许约 5-15 度自然偏角，不能转成相反方向、垂直方向、面对现有沙发或为了镜头改变朝向；第四步保持沙发位置和朝向不动；第五步只移动相机位置、焦段和裁切来满足当前视角。对于有大窗/窗帘的客厅，远景必须在原图核心空间结构内摆入沙发，不新增房间结构；中近景和近景可以通过相机远近、平移、旋转和局部裁切形成对应视角，背景可随机位自然变化。不要为了构图把沙发移到画面中央、茶几旁、电视前方或通道中央，也不要为了让产品正对镜头而旋转沙发本体。
