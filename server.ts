...
const FIXED_ROOM_SOFA_PLACEMENT =
  "唯一固定落点：如果参考房间里有现有沙发/长沙发，上传单人沙发只能放在现有沙发靠主窗一端的后侧外角，靠近原图已有主窗窗帘内侧与右侧画作墙/落地灯/边几交界的室内采光区；整把单人沙发必须位于现有沙发前沿线之后，也就是更靠窗、更靠墙、更靠后的位置，不得进入原沙发前沿线向相机、茶几或地毯中心延伸的前方区域。这个位置是唯一物理落点，不在原沙发左侧或右侧之间二选一，也不是地毯中央、茶几旁、原沙发正前方或前侧地面。";
...
function buildFallbackSceneAnalysis(viewMode: ViewMode, error?: any) {
  return {
    ...
    placementSuggestion: `${WINDOW_FRONT_PRIORITY} ${FIXED_ROOM_SOFA_PLACEMENT} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 按当前视角规则执行：${viewPrompts[viewMode]}`,
    recommendedScale: "根据参考房间中的长沙发、地毯、地砖缝、灯具、窗高、柜体和通道宽度判断真实单人椅比例。",
    recommendedOrientation: `如果有现有沙发，先识别现有沙发的前沿线、正面开口方向、主窗一端、后侧外角、主窗位置和周边物品尺度，再执行：${FIXED_ROOM_SOFA_PLACEMENT} 沙发斜向约 15-35 度自然摆放；禁止放到原沙发正前方或前侧地面，不能越过原沙发前沿。否则根据会客区、电视墙、地毯方向、地面透视线和可用动线决定沙发朝向。${SOFA_ORIENTATION_MATCH}`,
...
function buildScenePlacementContext(sceneAnalysis?: any) {
  ...
  return `场景分析落位参考（仅作为参考；如有冲突，必须执行统一硬规则：沙发固定在原图已有主窗/落地窗/窗帘旁的室内采光区；禁止新增第二扇窗、右侧落地窗、额外白纱帘或原图没有的窗墙。如果房间里有现有沙发，先判断现有沙发前沿线、正面向量、主窗一端、后侧外角和周边物品尺度，再执行：${FIXED_ROOM_SOFA_PLACEMENT} 沙发斜向约 15-35 度自然摆放；不要求 L 型，沙发可以不在图片中心，禁止为了居中放到原沙发正前方或前侧地面；生成时只移动相机位置、焦段和裁切。远景必须保留原图主视角和核心空间结构；中近景和近景只能裁切/平移原房间结构，不能重造窗户和墙面关系）：placementSuggestion=${placement}；recommendedScale=${scale}；recommendedOrientation=${orientation}。`;
}
...
const viewPrompts: Record<ViewMode, string> = {
  ...
  mid:
    `中近景 / 产品环境主视觉（完整产品优先）：目标是通过改变机位让沙发清楚可见，同时仍能看出它在原房间窗边的真实固定位置；不是全屋远景，也不是材质特写。中近景不得重新摆放沙发，必须继承已经确定的唯一固定物理落点和 15-35 度斜向朝向：${FIXED_ROOM_SOFA_PLACEMENT} 只允许改变相机位置、焦段、视角高度和裁切。先锁定原图已有主窗、窗帘、右侧画作墙、落地灯、边几、绿植和原沙发之间的真实关系；禁止新增第二扇窗、右侧落地窗、额外白纱帘或原图不存在的窗墙。沙发可以不在图片中心位置，宁可让产品不居中，也不能为了居中把它前移到原沙发前方。绝对不要把生成沙发放到原沙发正前方、前侧地面、茶几通道、地毯中央前景或视觉前景中，不能越过或压住原沙发前沿，哪怕一点点也不允许。不允许为了中近景构图、完整展示、产品正面或镜头角度移动沙发本体到原沙发前面。推荐 35-50mm 等效中焦，平视或略低平视，画面是相机看向唯一固定落点的产品环境视角。${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} 沙发必须完整可见但要比当前主视觉更小，占画面高度约 18%-30%、宽度约 15%-28%，靠背、扶手、坐垫、脚踏/底座和整体轮廓不能被裁掉。${WINDOW_SIDE_CAMERA} 背景范围是局部原房间环境，占画面约 55%-70%；必须保留原图主窗只有一组、右侧是画作墙和落地灯而不是窗户，参考地毯、地板、边几、灯具、绿植、已有沙发局部、电视墙、柜体、墙面、窗帘或窗户局部中的 2-4 类锚点来判断真实比例，不要把单椅放大成客厅中心大主体。窗户只需要证明沙发在原有窗边，可以是边缘、局部背景、柔和窗光或窗框线索；背景只能随相机裁切和平移变化，必须延续原房间结构、风格、材质和采光。视角边界：禁止全屋远景、产品过小、展示完整客厅结构、近景裁切、只拍扶手/坐垫/材质、背面主视角、房间中央摆放、原沙发前方摆放、茶几旁中心、电视前方、通道中央、新增第二扇窗、右侧落地窗、为了镜头旋转沙发、为了画面居中前移沙发、比例失真或纯窗外景。`,
  close:
...
新增元素：${(addedElements || []).join(", ") || "无"}。保持真实比例和原房间风格，不破坏主要空间关系，不遮挡沙发主体。`;
    ...
    return `${shared}
房间融入专用规则：${WINDOW_FRONT_PRIORITY} 不要求 L 型；必须执行唯一固定落点：${FIXED_ROOM_SOFA_PLACEMENT} 并参照绿植、边几、灯具、柜体、地砖和地毯尺寸把沙发缩小到真实单椅比例。${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 如果参考图原本不是当前机位，也必须优先满足“沙发在原图已有窗边、沙发不进入原沙发前方、只移动相机和裁切来形成当前视角”的关系。
  ...
  app.post("/api/analyze", async (req, res) => {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            {
              text:
                `请用中文分析这张室内场景图，用于生成单人沙发效果图。${analysisViewInstruction}聚焦于：1. 房间类型和装修风格；2. 门窗、阳台、墙地面、主要家具和动线；3. 光线方向、阴影软硬、色温和曝光；4. 相机高度、透视线、地面平面和尺度参照；5. 单人沙发最合理摆放位置、尺寸比例和自然朝向。placementSuggestion 必须建议：${WINDOW_FRONT_PRIORITY} ${FIXED_ROOM_SOFA_PLACEMENT} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 不要建议房间中央、原沙发正前方、前侧地面、茶几旁中心、地毯中央、通道中央或电视前方。recommendedOrientation 必须先描述图中现有沙发/长沙发的前沿线、正面开口方向、主窗一端、后侧外角和绿植/边几/灯具/柜体/地毯等尺度参照；如果图中有现有沙发/长沙发，上传单人沙发不要做 L 型，必须执行唯一固定落点，不允许写成左侧、右侧或侧后方多选，斜向约 15-35 度自然摆放，不能越过或压住原沙发前沿，哪怕一点点也不允许。产品可见性通过移动机位解决；如果画面看见产品正面或侧面，原因必须是相机移动到唯一固定落点后的合适角度，而不是把沙发挪到原沙发前方。只返回 JSON，字段包括 roomType、style、lighting、lightingDirection、colorPalette、cameraAngle、perspectiveCues、placementSuggestion、recommendedScale、recommendedOrientation、modelInteractionSuggestion、elements。elements 是需要保留的画面元素数组。`,
            },
      ...
      const parsed = readJsonBlock(response.text || "") || {
        ...
        placementSuggestion: `${WINDOW_FRONT_PRIORITY} ${FIXED_ROOM_SOFA_PLACEMENT} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA}`,
        recommendedScale: "与附近茶几、地毯、墙面高度保持合理比例",
        recommendedOrientation: `如果有现有沙发，先识别现有沙发的前沿线、正面开口方向、主窗一端、后侧外角和周边物品尺度，再执行：${FIXED_ROOM_SOFA_PLACEMENT} 沙发斜向约 15-35 度自然摆放；禁止放到原沙发正前方或前侧地面，产品可见性通过移动机位解决。如果没有现有沙发，则朝向房间主要活动区或电视墙。${SOFA_ORIENTATION_MATCH}`,
