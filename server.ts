...
新增元素：${(addedElements || []).join(", ") || "无"}。保持真实比例和原房间风格，不破坏主要空间关系，不遮挡沙发主体。`;
    ...
    return `${shared}
房间融入专用规则：${WINDOW_FRONT_PRIORITY} 不要求 L 型；必须执行唯一固定落点：${FIXED_ROOM_SOFA_PLACEMENT} 并参照绿植、边几、灯具、柜体、地砖和地毯尺寸把沙发缩小到偏小的真实单椅比例；整体只相当于长沙发一个单座位或更小一点，不能像双人位或客厅主沙发。${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 如果参考图原本不是当前机位，也必须优先满足“沙发在原图已有窗边、沙发不进入原沙发前方、只移动相机和裁切来形成当前视角”的关系。
  ...
  app.post("/api/analyze", async (req, res) => {
    try {
      ...
      const parsed = readJsonBlock(response.text || "") || {
        ...
        recommendedScale: "与附近茶几、地毯、墙面高度保持偏小的真实单人椅比例，不能接近双人位体量",
