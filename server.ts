...
const WINDOW_SIDE_CAMERA =
  "机位总规则：先固定沙发在窗户边的位置和朝向，再只移动相机位置、焦段、视角高度和裁切来形成当前视角；不要为了构图移动沙发本体。三个视角互斥：远景是全屋建立镜头，只允许接近原图主机位的轻微后退/轻微升高/轻微广角化；中近景是完整产品环境主视觉，相机只用于观察已经斜向摆在侧边/侧后方可用空位的单人沙发，不要把相机需求解释成把沙发前移到原沙发前方；近景是材质与结构特写，相机明显靠近并允许局部裁切。背景可以随机位自然变化，但变化必须来自相机位置变化和原房间结构延伸，不要凭空新增原图没有的大窗、墙面、房间开间或纯窗外景。";

const SOFA_ORIENTATION_MATCH =
  "已有沙发避前硬锁：如果参考房间里有现有沙发、长沙发、贵妃位或清晰座位，先识别现有沙发的正面向量、前沿线、左右侧边、侧后方空位、绿植/边几/灯具/柜体/地毯边界等尺度参照。新生成的单人沙发/单人椅不要求 L 型，也不要求与现有沙发平行或同向；只需要放在现有沙发左侧、右侧或侧后方的可用空位里，并以约 15-35 度自然斜向摆放，像真实室内陈列一样略微转角。沙发可以不在图片中心，可以明显偏左或偏右，构图中心不重要，避开原沙发前方才是最高优先级。禁止放在现有沙发正前方、前侧地面、茶几通道、地毯中央前景或视觉前景里，哪怕只压到原沙发前沿一点点也视为错误。不要为了让产品居中、填满画面或正对镜头而把沙发挪到原沙发前面；如果需要看到正面或侧面，只能移动相机到斜向侧边落位后的合适角度拍摄。产品比例必须参考原房间里的长沙发、绿植、落地灯、边几、窗高、地砖和地毯尺寸，生成得比当前主视觉更小、更像真实单椅，而不是占据客厅中心的大主体。只有在参考图没有可识别沙发时，才根据电视墙、会客区或主要活动区决定自然朝向。";
...
function buildFallbackSceneAnalysis(viewMode: ViewMode, error?: any) {
  return {
    ...
    recommendedOrientation: `如果有现有沙发，先识别现有沙发的前沿线、正面开口方向、左右侧边、侧后方空位和周边物品尺度，再把上传单人沙发放在现有沙发左侧、右侧或侧后方可用空位中，斜向约 15-35 度自然摆放；禁止放到原沙发正前方或前侧地面，不能越过原沙发前沿。否则根据会客区、电视墙、地毯方向、地面透视线和可用动线决定沙发朝向。${SOFA_ORIENTATION_MATCH}`,
...
function buildScenePlacementContext(sceneAnalysis?: any) {
  ...
  return `场景分析落位参考（仅作为参考；如有冲突，必须执行统一硬规则：沙发固定在窗户边/落地窗边/窗帘旁的室内采光区；如果房间里有现有沙发，先判断现有沙发前沿线、正面向量、左右侧边、侧后方空位和周边物品尺度，再让上传单人沙发落在现有沙发左侧、右侧或侧后方可用空位中，斜向约 15-35 度自然摆放；不要求 L 型，沙发可以不在图片中心，禁止为了居中放到原沙发正前方或前侧地面；生成时只移动相机位置、焦段和裁切。远景必须保留原图主视角和核心空间结构；只有中近景和近景可以让房间背景根据新机位、原房间风格和原结构自然变化）：placementSuggestion=${placement}；recommendedScale=${scale}；recommendedOrientation=${orientation}。`;
}
...
const viewPrompts: Record<ViewMode, string> = {
  ...
  mid:
    `中近景 / 产品环境主视觉（完整产品优先）：目标是让沙发清楚可见，同时仍能看出它在窗边房间里的真实位置；不是全屋远景，也不是材质特写。先识别原图中现有沙发的前沿线、正面开口方向、左右侧边、侧后方空位和周边物品尺度；生成的单人沙发/单人椅不要做 L 型，必须放在现有沙发左侧、右侧或侧后方可用空位中，并斜向约 15-35 度自然摆放。沙发可以不在图片中心位置，可以偏左、偏右或靠近画面边侧；宁可让产品不居中，也不能为了居中把它前移到原沙发前方。绝对不要把生成沙发放到原沙发正前方、前侧地面、茶几通道、地毯中央前景或视觉前景中，不能越过或压住原沙发前沿，哪怕一点点也不允许。再确认该侧边/侧后方落点仍位于窗户边/落地窗边/窗帘旁的室内采光区，然后只移动相机拍摄；不允许为了中近景构图移动沙发本体到原沙发前面。推荐 35-50mm 等效中焦，平视或略低平视，画面是斜向侧边摆放的产品环境视角。${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} 沙发必须完整可见但要比当前主视觉更小，占画面高度约 22%-34%、宽度约 18%-32%，靠背、扶手、坐垫、脚踏/底座和整体轮廓不能被裁掉。${WINDOW_SIDE_CAMERA} 背景范围是局部房间环境，占画面约 50%-65%；必须参考地毯、地板、边几、灯具、绿植、已有沙发局部、电视墙、柜体、墙面、窗帘或窗户局部中的 2-4 类锚点来判断真实比例，不要把单椅放大成客厅中心大主体。窗户只需要证明沙发在窗边，可以是侧后方、边缘、局部背景、柔和窗光或窗框线索；背景可以随机位变化，但必须延续原房间风格、材质和采光。视角边界：禁止全屋远景、产品过小、展示完整客厅结构、近景裁切、只拍扶手/坐垫/材质、背面主视角、房间中央摆放、原沙发前方摆放、茶几旁中心、电视前方、通道中央、为了镜头旋转沙发、为了画面居中前移沙发、比例失真或纯窗外景。`,
  close:
...
新增元素：${(addedElements || []).join(", ") || "无"}。保持真实比例和原房间风格，不破坏主要空间关系，不遮挡沙发主体。`;
    ...
    return `${shared}
房间融入专用规则：${WINDOW_FRONT_PRIORITY} 不要求 L 型；应优先在已有沙发左侧、右侧或侧后方可用空位中斜向摆放，参照绿植、边几、灯具、柜体、地砖和地毯尺寸把沙发缩小到真实单椅比例。${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 如果参考图原本不是当前机位，也必须优先满足“沙发在窗户边、沙发不进入原沙发前方、只移动相机和裁切来形成当前视角”的关系。
...
摆放逻辑：必须像真实室内设计师在现场布置。第一步先判断房间窗户、阳台、电视墙、通道、已有沙发/茶几/柜体的位置；第二步判断已有沙发的前沿线、正面开口方向、左右侧边、侧后方空位和周边物品尺度；第三步把上传单人沙发落在现有沙发左侧、右侧或侧后方可用空位中，斜向约 15-35 度自然摆放，并确认该落点仍位于窗户边/落地窗边/窗帘旁采光区；第四步保持沙发位置和避开原沙发前方的关系不动；第五步只移动相机位置、焦段和裁切来满足当前视角。对于有大窗/窗帘的客厅，远景必须在原图核心空间结构内摆入沙发，不新增房间结构；中近景和近景可以通过相机远近、平移、旋转和局部裁切形成对应视角，背景可随机位自然变化。不要为了构图把沙发移到画面中央、原沙发正前方、茶几旁、电视前方或通道中央，也不要为了让产品正对镜头而把沙发放大或前移。
