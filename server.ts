...
const viewPrompts: Record<ViewMode, string> = {
  ...
  mid:
    `中近景 / 产品环境主视觉（只改变机位，不改变沙发位置）：沙发物理位置、比例约束和斜向朝向必须完全继承已确定的唯一固定落点：${FIXED_ROOM_SOFA_PLACEMENT} 中近景只允许改变相机位置、焦段、视角高度和裁切，让沙发更清楚可见；绝不能重新摆放、前移、居中移动、旋转沙发本体或改变沙发与窗户、原沙发前沿线的相对关系。沙发仍然在原图已有主窗/窗帘前方的室内采光区，背景只能来自原房间结构的裁切和平移；必须保留原图主窗只有一组，右侧是画作墙和落地灯而不是窗户，禁止新增第二扇窗、右侧落地窗、额外白纱帘或原图不存在的窗墙。绝对不要把生成沙发放到原沙发正前方、前侧地面、茶几通道、地毯中央前景或视觉前景中，不能越过或压住原沙发前沿，哪怕一点点也不允许。推荐 35-50mm 等效中焦，平视或略低平视，相机看向窗前固定落点拍摄；沙发可以不在图片中心。${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 沙发必须完整可见但要比当前主视觉更小，占画面高度约 18%-30%、宽度约 15%-28%，靠背、扶手、坐垫、脚踏/底座和整体轮廓不能被裁掉。背景范围占画面约 55%-70%，参考地毯、地板、边几、灯具、绿植、已有沙发局部、电视墙、柜体、墙面、窗帘或窗户局部中的 2-4 类锚点判断真实比例。视角边界：禁止全屋远景、近景裁切、房间中央摆放、原沙发前方摆放、茶几旁中心、电视前方、通道中央、新增窗户、为了镜头旋转沙发、为了画面居中前移沙发、比例失真或纯窗外景。`,
  close:
