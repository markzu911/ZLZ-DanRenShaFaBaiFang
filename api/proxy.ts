...
function buildFallbackSceneAnalysis(viewMode: ViewMode, error?: any) {
  return {
    ...
    recommendedOrientation: `如果有现有沙发，先识别现有沙发的前沿线、正面开口方向、左右侧边、侧后方空位、主窗位置和周边物品尺度，再把上传单人沙发放在现有沙发靠窗一端的侧后方、边几/落地灯附近、原窗帘内侧附近或原沙发外侧边可用空位中，斜向约 15-35 度自然摆放；禁止放到原沙发正前方或前侧地面。${SOFA_ORIENTATION_MATCH}`,
...
async function handleAnalyze(req: any, res: any) {
  ...
  }。聚焦于：1. 房间类型和装修风格；2. 门窗、阳台、墙地面、主要家具和动线；3. 光线方向、阴影软硬、色温和曝光；4. 相机高度、透视线、地面平面和尺度参照；5. 单人沙发最合理摆放位置、尺寸比例和自然朝向。placementSuggestion 必须建议：${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 不要建议新增第二扇窗、右侧落地窗、额外白纱帘、房间中央、原沙发正前方、前侧地面、茶几旁中心、地毯中央、通道中央或电视前方。recommendedOrientation 必须先描述图中现有沙发/长沙发的前沿线、正面开口方向、左右侧边、靠窗一端侧后方空位、主窗位置和绿植/边几/灯具/柜体/地毯等尺度参照；如果有现有沙发/长沙发，上传单人沙发不要做 L 型，必须放在现有沙发靠窗一端侧后方、边几/落地灯附近、原窗帘内侧附近或原沙发外侧边可用空位中，斜向约 15-35 度自然摆放，不能越过或压住原沙发前沿，哪怕一点点也不允许。只返回 JSON，字段包括 roomType、style、lighting、lightingDirection、colorPalette、cameraAngle、perspectiveCues、placementSuggestion、recommendedScale、recommendedOrientation、modelInteractionSuggestion、elements。elements 是需要保留的画面元素数组。`;
