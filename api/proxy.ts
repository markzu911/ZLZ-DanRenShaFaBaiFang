...
const viewPrompts: Record<ViewMode, string> = {
  wide:
    `远景图：优先保留原图的主视角、空间尺度和主要建筑结构，像在原房间里真实摆入沙发，而不是重建一个新房间。不要新增窗户、扩大窗户、生成额外落地窗或把房间扩成原图不存在的左侧大窗空间；窗户数量、主窗位置、电视墙、原沙发区、茶几、地面反射和天花灯关系应尽量沿用原图。先在原图已有窗户/窗帘/窗边采光区附近确定沙发固定落点，再把相机轻微后退或使用较广角取景形成远景；远景只改变相机位置、焦段和取景范围，不改变沙发落点、朝向和与窗边采光区的真实空间关系。沙发是正常单人椅比例，占画面高度约 12%-20%、宽度约 10%-22%，完整可见但不主导画面。远景必须能判断沙发位于原房间的窗边采光区；窗户、窗帘或窗光可以出现在背景、侧边或边缘，比例根据原图结构和机位自然决定。避免产品特写、沙发过大、房间结构大幅改造、凭空增加窗户、把沙发放到房间中央或贴图感。${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA}`,
  mid:
