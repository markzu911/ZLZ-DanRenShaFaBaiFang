...
function buildFallbackSceneAnalysis(viewMode: ViewMode, error?: any) {
  return {
    ...
    recommendedOrientation: `如果有现有沙发，先识别现有沙发的前沿线、正面开口方向、左右侧边、侧后方空位和周边物品尺度，再让上传单人沙发落在现有沙发左侧、右侧或侧后方可用空位中，斜向约 15-35 度自然摆放；禁止放到原沙发正前方或前侧地面。${SOFA_ORIENTATION_MATCH}`,
...
摆放逻辑：先判断窗户、阳台、电视墙、通道、已有沙发/茶几/柜体的位置；如果有现有沙发，再判断现有沙发的前沿线、正面开口方向、左右侧边、侧后方空位和周边物品尺度，把上传单人沙发落在现有沙发左侧、右侧或侧后方可用空位中，并确认该落点仍位于窗户边/落地窗边/窗帘旁采光区，斜向约 15-35 度自然摆放；保持沙发位置和避开原沙发前方的关系不动，只移动相机位置、焦段和裁切来满足当前视角。远景必须在原图核心空间结构内摆入沙发，不新增房间结构；中近景和近景可以通过相机远近、平移、旋转和局部裁切形成对应视角，背景可随机位自然变化。禁止把生成沙发放到原沙发正前方、前侧地面、茶几旁、电视前方或通道中央。
