...
const WINDOW_FRONT_PRIORITY =
  "最高优先级落位：上传单人沙发必须物理落在窗户/落地窗/窗帘正前方的室内采光区，靠近窗墙、窗台、落地窗内侧或窗帘线。禁止把沙发放到房间中央、地毯中央、茶几旁中心、电视前方、柜门前方或通道中央。";

const WINDOW_SIDE_CAMERA =
  "机位规则：先固定沙发位置和朝向，再移动相机到窗边侧前方、同向沙发的正前方或侧前方。相机在室内靠近窗户/落地窗/窗帘一侧，镜头水平斜向室内，画面尽头应是屋内家具、墙面或会客区，不应是窗外或整面窗帘墙。";
...
const viewPrompts: Record<ViewMode, string> = {
  wide:
    `远景图：完整呈现房间、主要家具和沙发摆放关系。沙发是正常单人椅比例，占画面高度约 16%-24%，完整可见但不主导画面。${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA}`,
  mid:
    `中近景：相机更近、画面裁切更紧，沙发占画面高度约 32%-42%、宽度约 24%-34%。必须保留地毯、地板、边几、灯具、绿植、已有长沙发局部、电视墙、柜体、墙面或软装等室内锚点。${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA}`,
  close:
    `近景：突出沙发材质、轮廓、坐垫和局部软装细节，沙发主体占画面约 60%-72%，同时保留窗帘线、窗框边缘、窗台、窗墙或柔和窗光中的至少一个落位锚点。${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA}`,
};
...
房间融合规则：参考图用于提取房间类型、主要元素、装修风格、材质、采光和空间气质；最终应在不违反房间融入专用规则的前提下重新生成一张风格类似的完整室内场景，不需要逐像素复刻原图。
用于风格、元素和光线参考的场景分析：${JSON.stringify(safeSceneAnalysis)}
${scenePlacementContext}
${elementEditingRules}
图 1 是准确的沙发产品参考，图 2 是房间和空间参考。
摆放逻辑：先判断窗户、阳台、电视墙、通道、已有沙发/茶几/柜体的位置，并确定唯一固定沙发落点；如果有现有沙发，再判断现有沙发的大体朝向，把上传单人沙发的正面向量锁定为同一方向；保持沙发位置和朝向不动，只移动相机位置、焦段和裁切来满足当前视角。
