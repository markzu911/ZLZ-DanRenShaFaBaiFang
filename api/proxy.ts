...
function buildFallbackSceneAnalysis(viewMode: ViewMode, error?: any) {
  return {
    ...
    recommendedOrientation: `如果有现有沙发，先识别现有沙发的前沿线、正面开口方向、左右侧边和侧边可用空位，再让上传单人沙发落在现有沙发左侧或右侧边，与原沙发形成 L 型组合；禁止放到原沙发正前方或前侧地面。${SOFA_ORIENTATION_MATCH}`,
...
摆放逻辑：先判断窗户、阳台、电视墙、通道、已有沙发/茶几/柜体的位置；如果有现有沙发，再判断现有沙发的前沿线、正面开口方向、左右侧边和侧边可用空位，把上传单人沙发落在现有沙发左侧或右侧边，并确认该侧边落点仍位于窗户边/落地窗边/窗帘旁采光区，与原沙发形成 L 型组合；保持沙发位置和 L 型关系不动，只移动相机位置、焦段和裁切来满足当前视角。远景必须在原图核心空间结构内摆入沙发，不新增房间结构；中近景和近景可以通过相机远近、平移、旋转和局部裁切形成对应视角，背景可随机位自然变化。禁止把生成沙发放到原沙发正前方、前侧地面、茶几旁、电视前方或通道中央。
...
async function handleAnalyze(req: any, res: any) {
  ...
  }。聚焦于：1. 房间类型和装修风格；2. 门窗、阳台、墙地面、主要家具和动线；3. 光线方向、阴影软硬、色温和曝光；4. 相机高度、透视线、地面平面和尺度参照；5. 单人沙发最合理摆放位置、尺寸比例和自然朝向。placementSuggestion 必须建议：${WINDOW_FRONT_PRIORITY} ${SOFA_ORIENTATION_MATCH} ${WINDOW_SIDE_CAMERA} 不要建议房间中央、原沙发正前方、前侧地面、茶几旁中心、地毯中央、通道中央或电视前方。recommendedOrientation 必须先描述图中现有沙发/长沙发的前沿线、正面开口方向、左右侧边和侧边可用空位；如果有现有沙发/长沙发，上传单人沙发必须放在现有沙发左侧或右侧边，与原沙发形成 L 型组合，不能越过或压住原沙发前沿，哪怕一点点也不允许。只返回 JSON，字段包括 roomType、style、lighting、lightingDirection、colorPalette、cameraAngle、perspectiveCues、placementSuggestion、recommendedScale、recommendedOrientation、modelInteractionSuggestion、elements。elements 是需要保留的画面元素数组。`;
